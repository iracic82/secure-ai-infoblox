...
# === Step 2: Switch Account (using external_id.txt) ===
if not os.path.exists(EXTERNAL_ID_FILE):
    raise FileNotFoundError(f"❌ {EXTERNAL_ID_FILE} not found")

with open(EXTERNAL_ID_FILE, "r") as f:
    external_id = f.read().strip()

if not external_id:
    raise RuntimeError("❌ external_id.txt is empty")
...
print(f"🔁 Switched to account (external_id): {external_id}")
time.sleep(3)
